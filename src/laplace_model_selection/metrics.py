...
def calculate_laplace(model, pos_unscaled_map, variances_list=None, param_punish_term = -1.0, **kwargs):
    ...
    hessian_neg_unscaled_finite_differences = torch.tensor(finite_difference_hessian(model, model.likelihood, len(params_list), model.train_inputs[0], model.train_targets ) if bool_use_finite_difference_hessian else None)
    end = time.time()
...
def NestedSampling_call(model, **kwargs):
