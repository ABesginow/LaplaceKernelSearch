...
for n in list(range(1, 16)):
    ...
    aic = AIC()
    bic = BIC(len(X))
    nested_sampling = NestedSampling(model=model_MAP, prior=model_parameter_prior, store_full=True, logging=True, pickle_directory="./", pickle_name=f"nested_sampling_{n}.pkl", maxcall=1e+6, maxiter=1e+6)# print_progress=True, 
    ...
    for metric, metric_call in zip(target_metrics, metric_calls):
        ...
        if metric_log["name"] == "log MAP":
            metric_log["state_dict"] = copy.deepcopy(model_MAP.state_dict())
            metric_log["train_log"] = copy.deepcopy(training_log_MAP)
        if metric_log["name"] == "log ML":
            metric_log["state_dict"] = copy.deepcopy(model_MLL.state_dict())
            metric_log["train_log"] = copy.deepcopy(training_log_MLL)
        metric_log["logs"] = logs
        metric_log["model_evidence_approx"] = result
        metric_kut[n][str(metric)] = metric_log
...
# %%
import os
dill.dump(ALL_METRIC_LOGS, open(os.path.join("./", f"minimal_laplace_inconsistency_prod_manual.pkl"), "wb"))