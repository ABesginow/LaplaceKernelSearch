...
def plot_model(model, likelihood, X, Y, return_figure = False, figure = None, ax = None):
    interval_length = torch.max(X) - torch.min(X)
    shift = interval_length * options["plotting"]["border_ratio"]
    test_x = torch.linspace(torch.min(X) - shift, torch.max(X) + shift, options["plotting"]["sample_points"])
...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, kernel_text="RBF", weights=None):
        ...
        if kernel_text == "C*C*RBF":
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel()))
        elif kernel_text == "C*RBF":
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel())
        elif kernel_text == "4C*SIN":
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel())
        elif kernel_text == "C*SIN + C*SIN + C*SIN":
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel()) + gpytorch.kernels.ScaleKernel(gpytorch.kernels.PeriodicKernel())
...
def run_experiment(config):
    ...
    if config == "PER":
        ...
        observations_y = torch.sin(observations_x * 2*np.pi) * torch.exp(0.3*observations_x)
    ...
    for exp_num in range(EXPERIMENT_REPITITIONS):
        ...
        for model_kernel in model_kernels:
            ...
            model = ExactGPModel(observations_x, observations_y, likelihood, model_kernel)
...
#
            ...
            if "AIC" in metrics:
                AIC_approx, AIC_log = calculate_AIC(-loss * len(observations_x), sum(p.numel() for p in model.parameters() if p.requires_grad))
            ...
            if "Laplace" in metrics:
                laplace_approx, LApp_log = calculate_laplace(model, loss, param_punish_term=parameter_punishment)
                Laplace_logs = dict()
            ...
            # Perform MCMC
            if "MC" in metrics:
                MCMC_approx, MC_log = calculate_mc_STAN(model, likelihood, num_draws)
                MC_logs = dict()
